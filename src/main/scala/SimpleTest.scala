...
object SimpleTest {
  ...
  def main(args: Array[String]) {
    ...
    val yValues = xValues.map { x =>
      val square = x*x*x-280*x*x+21100*x-408000
      val range = square * 0.2
      val fluctuation = rnd.nextDouble()*range
      square + fluctuation - fluctuation/2 + 1200
    }
    val rdd = sc.parallelize((xValues, yValues).zipped.map{(x, y) => Row(1d, x, x*x, x*x*x, y)})
    ...
    val predictionColumns = List("const", "x", "x2", "x3")
